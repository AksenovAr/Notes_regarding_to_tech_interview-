...
typedef std::unique_ptr< input_stream_info > info_ptr;
...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i<10; ++i)
	{
		...
		info_ptr info( new input_stream_info(buffer_id, source_id));
		list_stream.insert( std::move(info));
	}
	...
	if (it != ls.end())
	{
		p_info = const_cast<input_stream_info*> (it->get());
